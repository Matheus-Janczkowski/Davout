...
\usepackage{mathrsfs}% F fresco da transformada de Fourier
%\usepackage{authblk}
\usepackage{accents}
\usepackage{hyperref}
...
\usepackage{xcolor}
\usepackage[nocn]{ffcode} % Pacote para fazer código estilo do terminal. O 'nocn' faz com que a numeração das linhas comece sempre em 1
\usepackage{colortbl}
\usepackage[shortlabels]{enumitem}% Package to enumerate with letters
\usepackage[section]{placeins}% Package to place figures within the sections where they had been already placed.
\usepackage{comment}
\usepackage{listings}
\lstset{
  basicstyle=\ttfamily,
...
% Calls our own package

\usepackage{LaTeXUtilities}

\usepackage{etoolbox}
\usepackage{caption}
\DeclareCaptionType{command}[Command][List of Commands]
...
% Macro para tirar o Preprint submitted to Elsevier
...
\author{Matheus Janczkowski Fogaça}
...
\begin{figure}[h!]%
\centering
\includegraphics[width=0.6\textwidth]{Figuras/aplicacoes_multiescala.pdf}
...
\cline{2-9}
\multirow{-2}{*}{\cellcolor{LightGray}{Atividades}} & \cellcolor{LightGray}{1\oCardinal$\;$Bi} & \cellcolor{LightGray}{2\oCardinal $\;$Bi} & \cellcolor{LightGray}{3\oCardinal $\;$Bi} & \cellcolor{LightGray}{4\oCardinal $\;$Bi} & \cellcolor{LightGray}{1\oCardinal $\;$Bi} & \cellcolor{LightGray}{2\oCardinal $\;$Bi} & \cellcolor{LightGray}{3\oCardinal $\;$Bi} & \cellcolor{LightGray}{4\oCardinal $\;$Bi} \\
\hline
