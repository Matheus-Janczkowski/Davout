# Routine to store tests for the FENN loss function
...
from ..optimization.objective_function import LossFunction
...
# those provided by FEniCS, then compares the loss function

def compare_loss_function(residual_vector, residual_vector_fenics,
assembled_residual, mesh_data_class, corresponding_realizations, 
test_name, loss_function_value, 
identity_conditioning_matrix_constants_array):
    ...
    print("\nLoss function: FENN -> "+str(
    loss_function_value.numpy())+"            FEniCS -> "+str(np.sum(
    identity_conditioning_matrix_constants_array*np.sum(
    residual_vector_fenics[:,corresponding_realizations]**2, axis=0))))
...
class TestANNTools:
    ...
    def __init__(self):
        ...
        # Selects the constant for the identity conditioning matrix

        self.identity_conditioning_matrix_constant = 1.0
    ...
    def test_material_parameters_with_single_conditioning_matrix(self):

        print("\n#####################################################"+
        "###################\n#        Tests loss function while batch"+
        "ing material parameters        #\n#                    with s"+
        "ingle conditioning matrix                   #\n##############"+
        "##########################################################\n", 
        flush=True)
        ...
        loss_function_instance = LossFunction({"conditioner name": "Id"+
        "entityMultiple", "multiple": 
        self.identity_conditioning_matrix_constant}, {"loss function n"+
        "ame": "QuadraticForm"}, vector_of_parameters)
        ...
        compare_loss_function(residual_vector, 
        self.residual_vector_fenics, self.assembled_residual, 
        self.mesh_data_class, corresponding_realizations, "BATCHED MAT"+
        "ERIAL PARAMETERS WITH SINGLE CONDITIONING MATRIX", 
        loss_function_instance.evaluate_loss(vector_of_parameters, 
        residual_vector), self.identity_conditioning_matrix_constant*
        np.ones((2)))

    # Defines a function to test batching material parameters

    def test_material_parameters_with_multiple_conditioning_matrices(
    self):

        print("\n#####################################################"+
        "###################\n#        Tests loss function while batch"+
        "ing material parameters        #\n#                  with mul"+
        "tiple conditioning matrices                 #\n##############"+
        "##########################################################\n", 
        flush=True)
        ...
        conditioning_constants = np.array([
        self.identity_conditioning_matrix_constant, 1.5*
        self.identity_conditioning_matrix_constant])

        conditioning_dict = [{"conditioner name": "IdentityMultiple", 
        "multiple": conditioning_constants[0]}, {"conditioner name": 
        "IdentityMultiple", "multiple": conditioning_constants[1]}]

        loss_function_instance = LossFunction(conditioning_dict, {"los"+
        "s function name": "QuadraticForm"}, vector_of_parameters)
        ...
        self.residual_vector_fenics, self.assembled_residual, 
        self.mesh_data_class, corresponding_realizations, "BATCHED MAT"+
        "ERIAL PARAMETERS WITH MULTIPLE CONDITIONING MATRICES", 
        loss_function_instance.evaluate_loss(vector_of_parameters, 
        residual_vector), conditioning_constants)
