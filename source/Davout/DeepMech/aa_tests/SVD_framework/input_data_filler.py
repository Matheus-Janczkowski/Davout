...
final_time):
    ...
    # than the input data
        ...
        # Initializes a list of samples that have already been filtered
        # from rigid-body motion

        new_filtered_input_list = []
        ...
        for i in range(input_data.shape[0]):
            ...
            for load_step in range(n_load_steps):
                ...
                # Adds this to the input data. Inserts the load-based 
                # data as the first columns, since this is the implemen-
                # tation convention of SVDQuotientSpace

                new_input_list.append([new_disp_grad[0,0], 
                new_disp_grad[0,1], new_disp_grad[0,2], new_disp_grad[
                1,0], new_disp_grad[1,1], new_disp_grad[1,2], 
                new_disp_grad[2,0], new_disp_grad[2,1], new_disp_grad[
                2,2], sample_data[0], sample_data[1]])

                # Adds this sample to the filtered input data. Inserts 
                # the load-based data as the first columns, since this 
                # is the implementation convention of SVDQuotientSpace.
                # Uses the annihilator mapping of the paper RP02
        ...
        new_filtered_input_list = np.array(new_filtered_input_list)

        print("The new filtered input data matrix has shape "+str(
        new_filtered_input_list.shape)+", whereas it had "+str(
        input_data.shape[0])+" rows before.\nThe displacement matrix, "+
        "on the other hand, has "+str(displacement_matrix.shape[0])+"\n")

        np.save(results_path+"//"+new_filtered_input_file, 
        new_filtered_input_list)
